...
static int gpio_get_clock(uint32_t gpio_number)
{
	...
	if ((MmioRead32(info->clkgate_reg) & (1U << info->clkgate_bit)) == 0U) {
		return 0;
	}
	MmioWrite32(
		info->clkgate_reg,
...
void gpio_put_clock(uint32_t gpio_number, uint32_t clock_state)
{
	...
	MmioWrite32(info->clkgate_reg, BITS_WITH_WMASK(1, 1, info->clkgate_bit));
}
...
static int get_pull(int gpio)
{
	...
	val = (MmioRead32(info->pull_base + 4 * bank) >> (id * 2)) & GPIO_P_MASK;
...
void plat_rockchip_save_gpio(void)
{
	...
	cru_gate_save = MmioRead32(CRU_BASE + CRU_CLKGATE_CON(31));
	 ...
	 * gating them
	 */
	MmioWrite32(CRU_BASE + CRU_CLKGATE_CON(31),
	...
	for (i = 0; i < ARRAY_SIZE(store_grf_gpio); i++)
		store_grf_gpio[i] =
			MmioRead32(GRF_BASE + GRF_GPIO2A_IOMUX + i * 4);
}
...
void plat_rockchip_restore_gpio(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(store_grf_gpio); i++)
		MmioWrite32(GRF_BASE + GRF_GPIO2A_IOMUX + i * 4,
	...
	for (i = 2; i < 5; i++) {
		...
		MmioWrite32(base + SWPORTA_DR, save->swporta_dr);
		MmioWrite32(base + SWPORTA_DDR, save->swporta_ddr);
		MmioWrite32(base + INTEN, save->inten);
		MmioWrite32(base + INTMASK, save->intmask);
		MmioWrite32(base + INTTYPE_LEVEL, save->inttype_level);
		MmioWrite32(base + INT_POLARITY, save->int_polarity);
		MmioWrite32(base + DEBOUNCE, save->debounce);
		MmioWrite32(base + LS_SYNC, save->ls_sync);
	}
	MmioWrite32(CRU_BASE + CRU_CLKGATE_CON(31),
			cru_gate_save | REG_SOC_WMSK);
}
