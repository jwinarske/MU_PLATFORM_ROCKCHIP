		    ...
		    int *panel_bits_per_colourp)
{
	...
	if (buf_size < sizeof(*edid) || edid_check_info(edid)) {
		debug(STR_FMT": Invalid buffer\n", __func__);
		return -EINVAL;
	}

	if (!EDID1_INFO_FEATURE_PREFERRED_TIMING_MODE(*edid)) {
		debug(STR_FMT": No preferred timing\n", __func__);
		return -ENOENT;
	}
	...
	if (!EDID1_INFO_VIDEO_INPUT_DIGITAL(*edid)) {
		debug(STR_FMT": Not a digital display\n", __func__);
		return -ENOSYS;
	}
	if (edid->version != 1 || edid->revision < 4) {
		debug(STR_FMT": EDID version %d.%d does not have required info\n",
...
void edid_print_info(struct edid1_info *edid_info)
{
	...
	printf("Video input definition: "STR_FMT"voltage level %d"STR_FMT""STR_FMT""STR_FMT""STR_FMT""STR_FMT"\n",
	...
	printf("Power management features: "STR_FMT""STR_FMT", "STR_FMT""STR_FMT", "STR_FMT""STR_FMT"\n",
