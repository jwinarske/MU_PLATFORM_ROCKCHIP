...
class CommonPlatform:
    ...
    """
    PackagesSupported = ("PinePhonePro",)
...
class SettingsManager(UpdateSettingsManager, SetupSettingsManager, PrEvalSettingsManager):
    ...
    def GetPlatformDscAndConfig(self) -> tuple:
        ...
        evaluate if any of the changes will be built in the dsc.

        The tuple should be (<workspace relative path to dsc file>,
        <input dictionary of dsc key value pairs>)
        """
        return "Pine64/PinePhonePro/PinePhonePro.dsc", {}
...
class PlatformBuilder(UefiBuilder, BuildSettingsManager):
    ...
    def SetPlatformEnv(self):
        ...
        self.env.SetValue("ACTIVE_PLATFORM", "Platforms/Pine64/PinePhonePro/PinePhonePro.dsc", "Platform Hardcoded")
        ...
        self.env.SetValue("CONF_AUTOGEN_INCLUDE_PATH",
                          self.mws.join(self.ws, "Platforms", "Pine64", "PinePhonePro", "Include"), "Platform Defined")

        self.env.SetValue("YAML_POLICY_FILE", self.mws.join(self.ws, "PinePhonePro", "PolicyData", "PolicyDataUsb.yaml"),
                          "Platform Hardcoded")
        self.env.SetValue("POLICY_DATA_STRUCT_FOLDER", self.mws.join(self.ws, "PinePhonePro", "Include"), "Platform Defined")
        self.env.SetValue('POLICY_REPORT_FOLDER', self.mws.join(self.ws, "PinePhonePro", "PolicyData"), "Platform Defined")
        self.env.SetValue('MU_SCHEMA_DIR', self.mws.join(self.ws, "Platforms", "Pine64", "PinePhonePro", "CfgData"),
                          "Platform Defined")
        self.env.SetValue('MU_SCHEMA_FILE_NAME', "PinePhoneProCfgData.xml", "Platform Hardcoded")
        self.env.SetValue('CONF_PROFILE_PATHS',
                          self.mws.join(self.ws, 'Platforms', 'Pine64', 'PinePhonePro', 'CfgData',
                                        'Profile0PinePhoneProCfgData.csv') + " " +
                          self.mws.join(self.ws, 'Platforms', 'Pine64', 'PinePhonePro', 'CfgData',
                                        'Profile1PinePhoneProCfgData.csv'),
                          "Platform Hardcoded"
                          )
