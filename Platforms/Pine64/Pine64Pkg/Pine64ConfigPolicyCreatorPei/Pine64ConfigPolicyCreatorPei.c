...
EFI_STATUS
EFIAPI
Pine64ConfigPolicyCreatorPeiEntry (
  ...
  )
{
  ...
  // Pine64 can choose to do any Pine64 specific things to config here such as enforcing static only config or
  ...
  // Policy Service will receive gPine64ConfigPolicyGuid and publish it as a PPI so that the Silicon Policy Creator can
  // have a depex on it and map it to Silicon Policies
  Status = PolPpi->SetPolicy (&gPine64ConfigPolicyGuid, POLICY_ATTRIBUTE_FINALIZED, ConfPolicy, ConfPolicySize);
